...
@Service
class CachingService(private val gitHubEnterpriseProperties: GitHubEnterpriseProperties) {
    ...
    private val cache: Cache<String, CacheValue> = Caffeine.newBuilder()
        ...
        .expireAfter(object : Expiry<String, CacheValue> {
            override fun expireAfterCreate(key: String, value: CacheValue, currentTime: Long): Long =
                TimeUnit.SECONDS.toNanos(value.expiry ?: Long.MAX_VALUE)
            ...
            ): Long = TimeUnit.SECONDS.toNanos(value.expiry ?: Long.MAX_VALUE)
    ...
    fun get(key: String): Any? = cache.getIfPresent(key)?.value
